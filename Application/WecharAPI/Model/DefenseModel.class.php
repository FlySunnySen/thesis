...
//			$rst3 = $Model->query($sql);
	...
	//按照学号添加学生
	public function findAddStudent($num_name,$defense_group,$year,$status)
	{
        ...
        $stuRst = $studentModel->where('Snum='.$num_name)->find();
		//判断该学生是否符合条件
		if(empty($stuRst)){		
			return '该学生不存在';
			
		}elseif($stuRst['year'] != $year){
			return '该学生不是当前年份的学生';
			
		}elseif($stuRst['denfense_status'] != $status){
			return '答辩状态不符合';
		}elseif($stuRst['denfense_allocation'] != 0){
			return '该学生已分配';
		}
	    ...
	    if($defense_student){
			...
			$rst2 = self::changeStatus($defense_student['student_Snum'],$status);			
