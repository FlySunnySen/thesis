	...
	<br />
		...
		<table id="myTable">
			...
			<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr class="text-center">
				...
				<td><a href="javascript:void(0);" onclick="savedStatus('<?php echo ($v["Snum"]); ?>','a_<?php echo ($v["Snum"]); ?>')" class="a_<?php echo ($v["Snum"]); ?>"><?php if(($v["denfense_status"]) == ""): ?>推荐<?php else: ?>已推荐<?php endif; ?></a></td>
...
<script>
	...
	function savedStatus(Snum,style){
		$.ajax({
			type:"post",
			url:"<?php echo U('myStudent/change');?>",
			dataType:"json",
			data:{Snum:Snum},
			success:function(data){
				alert(data.info);
				if(data.status==2){
					$(location).attr('href',"/biye/index.php/Teacher/Score/Ggrade");
				}
				if(data.status==1){									
				$('.'+style).html("已推荐");
				}
